use crate::core::{Extractor, VideoMetadata, VideoFormat, Thumbnail};
...
impl YouTubeExtractor {
    ...
    fn decrypt_signature(&mut self, signature: &str, js_content: &str) -> Result<String> {
        ...
        self.signature_decrypter.decrypt_signature(signature, js_content)
    }
    ...
    async fn process_cipher_format(&mut self, format: &Value, js_content: &str) -> Result<Option<String>> {
        // Handle signatureCipher or cipher formats
        let cipher = format.get("signatureCipher")
    ...
    async fn extract_metadata_with_js(&mut self, html: &str, video_id: &str, js_content: &str) -> Result<VideoMetadata> {
        ...
        let duration = video_details.get("lengthSeconds")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse::<u64>().ok());
        
        let uploader = video_details.get("author")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        ...
        let formats = self.extract_formats_with_js(&player_response, js_content).await?;
    ...
    async fn extract_formats_with_js(&mut self, player_response: &Value, js_content: &str) -> Result<Vec<VideoFormat>> {
    ...
    async fn parse_format_with_js(&mut self, format: &Value, js_content: &str) -> Result<Option<VideoFormat>> {
        // Try to get direct URL first
        let url = format.get("url").and_then(|v| v.as_str()).map(|s| s.to_string());
        
        // If no direct URL, try to process cipher
        let final_url = match url {
            Some(url) => url,
            None => {
                match self.process_cipher_format(format, js_content).await? {
                    Some(url) => url,
                    None => return Ok(None),
                }
            }
        ...
        let itag = format.get("itag")
            .and_then(|v| v.as_i64())
            .map(|i| i.to_string())
            .unwrap_or_else(|| "unknown".to_string());
        
        let quality = format.get("quality")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        
        let width = format.get("width").and_then(|v| v.as_i64()).map(|i| i as u32);
        let height = format.get("height").and_then(|v| v.as_i64()).map(|i| i as u32);
        ...
        let filesize = format.get("contentLength")
            .and_then(|v| v.as_str())
            .and_then(|s| s.parse::<u64>().ok());
...
impl Extractor for YouTubeExtractor {
    ...
    async fn extract(&mut self, url: &Url) -> Result<VideoMetadata> {
        ...
        let metadata = self.extract_metadata_with_js(&html, &video_id, &js_content).await?;
